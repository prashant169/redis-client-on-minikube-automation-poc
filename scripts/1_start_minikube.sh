...
if ! command -v kubectl &> /dev/null; then
  ...
  echo "🛠️ Adding $KUBECTL_DIR to system PATH..."
  powershell.exe -Command "[Environment]::SetEnvironmentVariable('Path', [Environment]::GetEnvironmentVariable('Path', 'Machine') + ';C:\\Program Files\\Kubernetes', 'Machine')" || echo "⚠️ Failed to update PATH for kubectl"
else
...
if ! command -v minikube &> /dev/null; then
  ...
  echo "🛠️ Adding $MINIKUBE_DIR to system PATH..."
  powershell.exe -Command "[Environment]::SetEnvironmentVariable('Path', [Environment]::GetEnvironmentVariable('Path', 'Machine') + ';C:\\Program Files\\Minikube', 'Machine')" || echo "⚠️ Failed to update PATH for Minikube"
else
